 ...
 *
 * Exported standalone (not just as a hook) so non-component callers — like
 * the AI chat tool executors — can reuse the exact same logic.
 */
export async function computeLostFollowerEvents(): Promise<LostFollowerEvent[] | undefined> {
  if (typeof window === "undefined") return undefined;
  const db = getDb();
  const snapshots = (await db.snapshots.toArray()).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
...
export function useLostFollowerEvents(): LostFollowerEvent[] | undefined {
  return useLiveQuery(computeLostFollowerEvents, []);
}
